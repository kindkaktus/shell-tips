...
### Replacing endlines in text file
...
*NOTICE: never use the same input and output file in the constructions like ./some-app < file.log > file.log
because the output file would be truncated before ./some-app gets invoked leavig you with an empty file as a result*
...
```
  ...
  - Inspect partition range for the newly added disk space (`F`)
  - Create a new primary partition (`n` and `p`)
  - [is it necessary?] Change the partition type to Linux LVM (`t` and `8e`)
  - Apply the changes (`w`)
  - Restart once completed

4. Extend volume group with the added partition (imagine the partition you just added is `/dev/sda3` and your LVM volume group reported by `vgs` is `ubuntu-vg`)
```
...
- `openssl x509 -outform der -in cert.pem -out cert.der` - convert PEM to DER
