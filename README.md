...
- `ln -snf /path/to/target-directory linkname` - CORRECT way to create a symlink to a directory (NOT `ln -sf` as the latter, provided 'linkname' already exists, will create /path/to/target-directory/target-directory symlink pointing to /path/to/target-directory)
